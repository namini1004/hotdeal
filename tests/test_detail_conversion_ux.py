...
class DetailConversionUxTests(unittest.TestCase):
    ...
    def test_detail_renders_bootstrap_before_loading_full_detail(self):
        ...
        self.assertNotIn('if(cachedDetail){', html)
        self.assertIn('const latest = await fetchItemById(id);', html)
        self.assertIn('if(error?.status === 404){', html)
        self.assertIn('clearCachedDetail(id);', html)
        self.assertIn('removeItemIdFromCache(id);', html)
        self.assertIn('renderNotFound();', html)
