...
class ImageVariantTests(unittest.TestCase):
    def test_make_webp_image_respects_requested_max_size(self):
        ...
        detail = sync.make_webp_image(content, 480)
        ...
        with Image.open(io.BytesIO(detail)) as detail_image:
            ...
            self.assertLessEqual(max(detail_image.size), 480)
    ...
    def test_mirror_feed_image_uploads_thumb_and_detail_variants(self):
        ...
        self.assertEqual(result["detail_img"], "https://example.supabase.co/storage/v1/object/public/deal-images/ppomppu/707448-detail-d46d478d2f62.webp")
        self.assertEqual(len(uploaded), 2)
        self.assertTrue(uploaded[0][0].endswith("/ppomppu/707448-thumb-d46d478d2f62.webp"))
        self.assertTrue(uploaded[1][0].endswith("/ppomppu/707448-detail-d46d478d2f62.webp"))
